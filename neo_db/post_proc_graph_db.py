...
def get_node_relation_no_id(node_name):
...
def get_node_no_cal_id(node_name):
    node = graph.run("""MATCH (n) 
                        WHERE n.Name='{0}' 
                        RETURN labels(n) AS labels, n AS properties""".format(node_name))
    data = node.next()
...
# 为了保持接口一致创建的简单函数
def no_update_id(json_data):
    return json_data

# 从Neo4j数据库生成relation3.json
def post_proc_graph_db(get_node_fcn, get_node_relation_fcn, update_id_fcn):
    nodes = get_all_nodes()
    json_data = []
    for record in nodes:
        node = get_node_fcn(record['n.Name'])
        relations = get_node_relation_fcn(record['n.Name'])
        data = {'node': node, 'relations': relations}
        json_data.append(data)

    new_json_data = update_id_fcn(json_data)
...
if __name__ == '__main__':
    ...
    # 用于初始创建Neo4j数据库时使用(自动更具节点属性计算节点ID)
    # post_proc_graph_db(get_node_cal_id, get_node_relation_no_id, update_id)
    # 用于已经存在节点ID的Neo4j数据库数据导出使用
    post_proc_graph_db(get_node_no_cal_id, get_node_relation_with_id, no_update_id)
